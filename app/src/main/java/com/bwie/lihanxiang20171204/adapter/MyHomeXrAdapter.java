...
import com.bwie.lihanxiang20171204.utils.GlideImaGlideImageLoader;
...
public class MyHomeXrAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    ArrayList<UserBean.DataBean.DefaultGoodsListBean> special_lists2;   // 热门专题的图片2
    ...
    public MyHomeXrAdapter(UserBean homeBean, Context context) {
...
//        intent = new Intent(context, DataWebViewActivity.class);
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == Item_Type.TYPE_ONE.ordinal()) {
            view = LayoutInflater.from(context).inflate(R.layout.nei_item01, null);
            return new ViewHolderA(view);
        } else if (viewType == Item_Type.TYPE_TWO.ordinal()) {
            view = LayoutInflater.from(context).inflate(R.layout.nei_item02,parent, false);
            return new ViewHolderB(view);
        } else if (viewType == Item_Type.TYPE_THREE.ordinal()) {
            view = LayoutInflater.from(context).inflate(R.layout.nei_item03, null);
            return new ViewHolderC(view);
        } else if (viewType == Item_Type.TYPE_FOUR.ordinal()) {
            view = LayoutInflater.from(context).inflate(R.layout.nei_item04, null);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ViewHolderA) {
            ...
            for (int i = 0; i < homeBean.getData().getActivityInfo().getActivityInfoList().size(); i++) {
                UserBean.DataBean.ActivityInfoBean.ActivityInfoListBean listBean = homeBean.getData().getActivityInfo().getActivityInfoList().get(i);
            ...
            for (int i = 0; i < homeBean.getData().getSubjects().size(); i++) {
                UserBean.DataBean.SubjectsBean sb = homeBean.getData().getSubjects().get(i);
                special_lists.add(sb.getImage());
            }
            for (int i = 0; i < homeBean.getData().getDefaultGoodsList().size(); i++) {
                UserBean.DataBean.DefaultGoodsListBean bean = homeBean.getData().getDefaultGoodsList().get(i);
                special_lists2.add(new UserBean.DataBean.DefaultGoodsListBean(bean.getGoods_name(), bean.getGoods_img()));
            }
            vb.recyclerView.setLayoutManager(new StaggeredGridLayoutManager(1, StaggeredGridLayoutManager.HORIZONTAL));
